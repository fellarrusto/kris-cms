<?php if (!defined('KRIS_EDITOR')) { http_response_code(404); exit; } // file interno ?>
    <div class="container">
        <h1>
            <?= htmlspecialchars(ucfirst((string)$group)) ?>
            <div class="actions">
                <a href="?action=structure&group=<?= urlencode((string)$group) ?>" class="btn btn-white">
        ...
        <div class="card">
            <?php if (empty($models[$group])): ?>
                <div style="padding:40px; text-align:center; color:#6b7280;">
                    ...
                    <a href="?action=structure&group=<?= urlencode((string)$group) ?>" class="btn btn-primary">Definisci Struttura</a>
                </div>
            <?php else: ?>
                            ...
                            foreach ($list as $item):
                                ...
                                foreach ($item['data'] as $d) {
                                    if (in_array($d['type'], ['text', 'plain', 'richtext'])) {
                                        ...
                                        if ($v) {
                                            $prev = htmlspecialchars(mb_substr(strip_tags($v), 0, 70)) . '...';
                                            break;
                                        }
                                    }
                                }
                                ...
                                <tr draggable="true" data-id="<?= $item['id'] ?>">
                                    ...
                                    <td style="text-align:right;">
                                        <a href="?action=edit&group=<?= urlencode((string)$group) ?>&id=<?= $item['id'] ?>" class="btn btn-white"
                                            style="padding:6px 10px;">Edit</a>
                                        <form method="POST" style="display:inline"
                                            onsubmit="return confirm('Eliminare definitivamente questo elemento?');">
                                            <input type="hidden" name="group" value="<?= htmlspecialchars((string)$group) ?>"><input type="hidden"
