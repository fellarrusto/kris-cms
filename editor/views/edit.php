    ...
    <div class="container">
        ...
        <form method="POST" class="card">
            <div class="card-body">
                ...
                <input type="hidden" name="group" value="<?= htmlspecialchars((string)$group) ?>">
                        ...
                        <?php if ($t === 'array'):
                            ...
                            <div style="background:#f9fafb; border:1px solid var(--border); border-radius:6px; padding:15px;">
                                ...
                                <?php else: ?>
                                            ...
                                            <?php foreach ($children as $ch):
                                                ...
                                                foreach ($ch['data'] ?? [] as $d) {
                                                    if (in_array($d['type'] ?? '', ['text','plain','richtext'])) {
                                                        ...
                                                        if ($v) { $prev = htmlspecialchars(mb_substr(strip_tags((string)$v), 0, 70)) . '...'; break; }
                                                    }
                                                }
                                                ...
                                                <tr draggable="true" data-id="<?= $chId ?>">
                                                    ...
                                                    <td style="text-align:right;">
                                                        <a href="?action=edit&group=<?= urlencode((string)$group) ?>&id=<?= $id ?>&path=<?= urlencode($childPath) ?>"
                                    ...
                                    <?php // Il form di riordino sta FUORI da questo form (vedi in fondo):
                                          // un <form> annidato e HTML non valido e il browser chiuderebbe
                                          // in anticipo il form di modifica, lasciando fuori il pulsante Salva. ?>
                                <?php endif; ?>
                            </div>

                        <?php elseif ($t === 'plain'): ?>
                            <textarea name="f[<?= $n ?>][val]" rows="2"><?= htmlspecialchars(is_array($saved) ? '' : (string) $saved) ?></textarea>
                                    ...
                                    <div class="tab-content group-<?= $n ?> <?= $i == 0 ? 'active' : '' ?>" id="<?= $n ?>_<?= $l ?>">
                                        <?php if ($t === 'image'): ?>
                                            <div style="display:flex; gap:10px; align-items:center;">
                                                <input type="text" name="f[<?= $n ?>][<?= $l ?>]" value="<?= htmlspecialchars((string)$v) ?>"
                                        ...
                                        <?php elseif ($t === 'richtext'): ?>
                                            <textarea name="f[<?= $n ?>][<?= $l ?>]" class="richtext"><?= htmlspecialchars($v) ?></textarea>

                                        <?php else: ?>
                                            <textarea name="f[<?= $n ?>][<?= $l ?>]" rows="4"
                                                style="min-height:100px;"><?= htmlspecialchars((string) $v) ?></textarea>
                                        <?php endif; ?>
                                    </div>
                                <?php endforeach; ?>
                            </div>
        ...
        foreach ($schema as $f):
            ...
            <?php if (!empty($children)): ?>
                <form method="POST" action="<?= htmlspecialchars($BASE) ?>" id="reorder_nested_<?= $n ?>"
                    class="reorder-nested-form" style="display:none;" data-field="<?= $n ?>">
